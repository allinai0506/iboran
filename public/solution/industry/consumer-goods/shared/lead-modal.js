(function () {
  ...
  modalForm.addEventListener("submit", async (event) => {
    ...
    if (phone?.value.trim() && !/^1[3-9]\d{9}$/.test(phone.value.trim()) && !invalid.includes(phone)) {
      invalid.push(phone);
    }
    ...
    if (invalid.length) {
      ...
      setHint("请先补充企业名称、联系人和有效的手机号。", "error");
      invalid[0].focus();
      return;
    }
    ...
    try {
      const payload = {
        name: modalForm.elements.contact.value.trim(),
        company: modalForm.elements.company.value.trim(),
        phone: modalForm.elements.phone_wechat.value.trim(),
        source: "consumer-goods-page-modal",
        sourcePath: "/solution/industry/consumer-goods",
        sourcePageUrl: window.location.href,
        remark: modalForm.elements.remark?.value?.trim() || "",
      };
      ...
      if (res.ok && data.success) {
        ...
        setHint(data.error || data.message || "提交失败，请稍后重试。", "error");
      }
    } catch (err) {
      setHint("提交失败，请稍后重试。", "error");
    } finally {
